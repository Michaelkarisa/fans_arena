...
import '../../joint/filming/screens/filmlayout2.dart';
...
class Stream2 extends StatefulWidget {
  EventM event;
  Stream2({super.key,required this.event});
...
class _Stream2State extends State<Stream2> {
  ...
  String message4='invited you to assist in filming their Event';
  ...
  Future<void> postNotifications(List<String> selectedStreamerIds,) async {
    try {
      for(final teamId in selectedStreamerIds) {
        await Sendnotification(message: message4,
            from: FirebaseAuth.instance.currentUser!.uid,
            to: teamId, content: widget.event.eventId,collection: "Professional").sendnotification();
      }
    } catch (e) {
  ...
  Future<void> addStreamer() async {
    try {
      CollectionReference streamersRef = FirebaseFirestore.instance.collection('Events').doc(widget.event.eventId).collection('streamers');
  ...
  Future<void> addStreamers() async {
    ...
    try {
      ...
      for (final teamId in allLikes1) {
        ...
        print('Added document for teamId: $teamId');
      }
      if (addedStreamerIds.isNotEmpty) {
        await postNotifications(addedStreamerIds);
        await NotifyFirebase().sendStreamingInvite(addedStreamerIds,FirebaseAuth.instance.currentUser!.uid,'event',widget.event.eventId);
      }
      await addStreamer();
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => AcceptedStreamers(event: widget.event),
        ),
      );
    } catch (e) {
...
class AcceptedStreamers extends StatefulWidget {
  EventM event;
  AcceptedStreamers({super.key, required this.event});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          SizedBox(
            ...
            child: TextButton(onPressed: (){
              Navigator.push(context,
                MaterialPageRoute(builder: (context) => FilmingLayout2(event:widget.event),
                ),
              );
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: _firestore.collection('Events').doc(widget.event.eventId).collection('streamers').snapshots(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return const Center(child: CircularProgressIndicator()); // Loading indicator
          }

          List<Streamers> notifications = snapshot.data!.docs.map(
                (doc) {
