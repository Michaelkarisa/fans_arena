...
import '../../appid.dart';
import '../../professionals/screens/genrescreen.dart';
...
class SignupscreenF1 extends StatefulWidget {
  const SignupscreenF1({super.key});

  @override
  State<SignupscreenF1> createState() => _SignupscreenF1State();
}
class _SignupscreenF1State extends State<SignupscreenF1> {
  ...
  Future<void> _saveEmailToCache(String email) async {
  ...
  Future<void> _register(BuildContext context) async {
    ...
    final favourite=genrecontroller.text;
    showDialog(
      context: context,
    ...
    try {
      ...
      if (year != null && month != null && day != null) {
        ...
        await FirebaseFirestore.instance.collection('Fans').doc(user.uid).set({
          'username': username,
          'fcmToken':fCMToken,
        ...
        _setCname(username);
      await FirebaseFirestore.instance.collection('Fans').doc(user.uid).collection('locations').add({
        'location': [like]
      });
  ...
  Future<UserCredential?> handleSignIn() async {
    ...
    try {
      ...
      if (googleUser != null) {
        ...
        final DocumentSnapshot doc = await FirebaseFirestore.instance.collection('Fans').doc(userId).get();
        if (doc.exists) {
          await back();
          ...
          await _firestore.collection('Fans').doc(userCredential.user!.uid).set({
            'username': username,
            'searchname':username.toLowerCase(),
            'email': email,
            'fcmToken':fCMToken,
          ...
          _setCname(username);
          await FirebaseFirestore.instance.collection('Fans').doc(userCredential.user!.uid).collection('locations').add({
            'location': [like]
          ...
          key: formKey,
              child: Column(
                children: [
                  ...
                  const Text('Fans Account',style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold,),),
                    ...
                    child: ClipRRect(
                      ...
                      child: InkWell(
                        ...
                        if (userCredential != null) {
                          Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => ContifP(userId:userCredential.user!.uid, username: userCredential.user!.displayName!,)),
                          );
                        }},
...
class ContifP extends StatefulWidget {
  final String userId;
  final String username;
  const ContifP({super.key, required this.userId,required this.username});

  @override
  State<ContifP> createState() => _ContifPState();
}

class _ContifPState extends State<ContifP> {
  String? _selectedGender;
  final FirebaseAuth _auth = FirebaseAuth.instance;
      ...
      try {
        ...
        if (querySnapshot.docs.isNotEmpty) {
          ...
          if (year != null && month != null && day != null) {
            ...
            if (wholeDate
                .toIso8601String()
                .isNotEmpty && wholeDate != oldData['birthday']) {
          ...
          if (_usernameController.text.isNotEmpty &&
              _usernameController.text != oldData['username']) {
          ...
          if (genrecontroller.text.isNotEmpty &&
              genrecontroller.text != oldData['genre']) {
