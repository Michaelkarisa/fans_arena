...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _sendAnalyticsEvent() async {
    await eventLogger.logCustomEvent('test_event', {
      'string': 'string',
      'int': 42,
      'long': 12345678910,
      'double': 42.0,
...
class EventLogger {
  ...
  Future<void> UserLogin(String loginMethod, String userId) async {
    Position location = await getCurrentLocation();
    await _analytics.logLogin(loginMethod: loginMethod, parameters: {
      'time': Timestamp.now(),
  ...
  Future<void> logButtonPress(String buttonName, String buttonFunction) async {
  ...
  Future<void> logCustomEvent(String buttonName, Map<String,Object> data) async {
    await _analytics.logEvent(
      name: 'buttonpress',
      parameters: data,
    );
  ...
  Future<void> UserSignUp(String signUpMethod, String userId) async {
    Position location = await getCurrentLocation();
    await _analytics.logSignUp(signUpMethod: signUpMethod, parameters: {
      'time': Timestamp.now(),
      'longitude': location.longitude,
      'latitude': location.latitude,
      'userId': userId,
  ...
  Future<void> openApp() async {
    Position location = await getCurrentLocation();
    await _analytics.logAppOpen(callOptions: AnalyticsCallOptions(global: false), parameters: {
      'time': Timestamp.now(),
      'longitude': location.longitude,
      'latitude': location.latitude,
    });
  ...
  Future<void> screenView(String screenClass, String screenName, DateTime startTime) async {
    final timeSpentInSeconds = DateTime.now().difference(startTime).inSeconds;
    final newHoursSpent = timeSpentInSeconds / 3600;
    Position location = await getCurrentLocation();
    await _analytics.logScreenView(screenClass: screenClass, screenName: screenName, parameters: {
      'time': Timestamp.now(),
      'longitude': location.longitude,
      'latitude': location.latitude,
